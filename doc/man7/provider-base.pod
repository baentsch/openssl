...
This value is used to refer to the algorithm within OpenSSL as its short name.
This value is mandatory to be supplied.

=item "tls-sigalg-longname" (B<OSSL_CAPABILITY_TLS_SIGALG_LONGNAME>) <UTF8 string>

The long name of the signature algorithm as used by OBJ_create().
...
The name of a Key Management algorithm that should be used with this
algorithm. The algorithm must support key and parameter generation.
This value is mandatory to be supplied.

=item "tls-sigalg-oid" (B<OSSL_CAPABILITY_TLS_SIGALG_OID>) <UTF8 string>

The OID of the signature algorithm.
This value is optional to be supplied. If supplied, the OID is automatically
registered for use. If not supplied, the provider must register the OID itself,
e.g., via core_obj_create().
...
This value is optional to be supplied. If not supplied, or if using the
value "" (empty string) the signature algorithm must be able to process
arbitrary length data in sign/verify operations.
