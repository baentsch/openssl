 ...
 */
int mlkem_encap_external_entropy(uint8_t *out_ciphertext,
                                  ...
                                  ossl_mlkem_ctx *mlkem_ctx)
{
    if (!mlkem_parse_public_key_no_hash(pub, in, mlkem_ctx))
        return 0;
    hash_h(pub->public_key_hash, in, OSSL_MLKEM768_PUBLIC_KEY_BYTES, mlkem_ctx);
    return 1;
}
...
int mlkem_parse_private_key(private_key_RANK768 *priv, uint8_t *in,
                            ossl_mlkem_ctx *mlkem_ctx) {
    uint8_t *in_curr = in;
