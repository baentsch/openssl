...
HYBRID_KEY *ossl_hybrid_key_new(OSSL_LIB_CTX *libctx,
                                const char *keytype1,
                                const char *keytype2,
                                const char *propq)
{
    HYBRID_KEY *ret = OPENSSL_zalloc(sizeof(*ret));

    if (ret == NULL)
        return NULL;

    ret->libctx = libctx;
    ret->keytype1 = OPENSSL_strdup(keytype1);
    ret->keytype2 = OPENSSL_strdup(keytype2);

    if (ret->keytype1 == NULL || ret->keytype2 == NULL)
        goto err;
...
err:
    ...
    OPENSSL_free(ret->keytype1);
    OPENSSL_free(ret->keytype2);
    OPENSSL_free(ret);
    return NULL;
}
...
void ossl_hybrid_key_free(HYBRID_KEY *key)
{
    ...
    REF_PRINT_COUNT("HYBRID_KEY", key);
    if (i > 0)
        return;
    REF_ASSERT_ISNT(i < 0);

    OPENSSL_free(key->propq);
    OPENSSL_free(key->keytype1);
    OPENSSL_free(key->keytype2);
...
HYBRID_KEY *ossl_hybrid_key_dup(const HYBRID_KEY *key, int selection)
{
    ...
    ret->keytype1 = OPENSSL_strdup(key->keytype1);
    ret->keytype2 = OPENSSL_strdup(key->keytype2);

    if (ret->keytype1 == NULL || ret->keytype2 == NULL)
        goto err;

    if (key->key1) {
        ret->key1 = evp_keymgmt_dup(key->key1->keymgmt,
                                    key->key1, selection);
        if (ret->key1 == NULL)
            goto err;
    }
    ...
    if (key->key2) {
        ret->key2 = evp_keymgmt_dup(key->key2->keymgmt,
                                    key->key2, selection);
        if (ret->key2 == NULL)
            goto err;
    }
...
/* TODO implement/enable eventually when adding encoding/decoding
