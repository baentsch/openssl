...
void *xor_prov_import_key(const OSSL_DISPATCH *fns, void *provctx,
...
void xor_prov_free_key(const OSSL_DISPATCH *fns, void *key)
{
    OSSL_FUNC_keymgmt_free_fn *kmgmt_free = xor_prov_get_keymgmt_free(fns);
...
#define XORSIGALG_LNAME XORSIGALG_NAME
#define XORSIGALG_OID "1.2.3.4.5.6.7.8"
#define XORSIGALG_NAME_INTERNAL "xorhmacsig-int"
#define XORSIGALG_ALGORITHM "xorhmacsig"
#define XORSIGALG_HASH_NAME "xorhmacsha2sig"
#define XORSIGALG_HASH_LNAME "xorhmacsha2sig-longtestname"
#define XORSIGALG_HASH_NAME_INTERNAL "xorhmacsha2sig-int"
#define XORSIGALG_HASH_ALGORITHM "xorhmacsha2sig"
#define XORSIGALG_HASH "SHA256"
#define XORSIGALG_HASH_OID "1.2.3.4.5.6.7.9"
...
typedef struct {
    OSSL_LIB_CTX *libctx;
    BIO_METHOD *corebiometh; /* needed for en/decode */
} PROV_XOR_CTX;
...
static int xor_prov_bio_from_dispatch(const OSSL_DISPATCH *fns)
{
    for (; fns->function_id != 0; fns++) {
...
static int xor_prov_bio_read_ex(OSSL_CORE_BIO *bio, void *data, size_t data_len,
                          size_t *bytes_read)
{
    if (c_bio_read_ex == NULL)
        return 0;
...
static int xor_prov_bio_write_ex(OSSL_CORE_BIO *bio, const void *data, size_t data_len,
                           size_t *written)
{
    if (c_bio_write_ex == NULL)
        return 0;
...
static int xor_prov_bio_gets(OSSL_CORE_BIO *bio, char *buf, int size)
{
    if (c_bio_gets == NULL)
        return -1;
...
static int xor_prov_bio_puts(OSSL_CORE_BIO *bio, const char *str)
{
    if (c_bio_puts == NULL)
        return -1;
...
static int xor_prov_bio_ctrl(OSSL_CORE_BIO *bio, int cmd, long num, void *ptr)
{
    if (c_bio_ctrl == NULL)
        return -1;
...
static int xor_prov_bio_up_ref(OSSL_CORE_BIO *bio)
{
    if (c_bio_up_ref == NULL)
        return 0;
...
static int xor_prov_bio_free(OSSL_CORE_BIO *bio)
{
    if (c_bio_free == NULL)
        return 0;
...
static int bio_core_read_ex(BIO *bio, char *data, size_t data_len,
                            size_t *bytes_read)
{
    return xor_prov_bio_read_ex(BIO_get_data(bio), data, data_len, bytes_read);
}

static int bio_core_write_ex(BIO *bio, const char *data, size_t data_len,
                             size_t *written)
{
    return xor_prov_bio_write_ex(BIO_get_data(bio), data, data_len, written);
}

static long bio_core_ctrl(BIO *bio, int cmd, long num, void *ptr)
{
    return xor_prov_bio_ctrl(BIO_get_data(bio), cmd, num, ptr);
}

static int bio_core_gets(BIO *bio, char *buf, int size)
{
    return xor_prov_bio_gets(BIO_get_data(bio), buf, size);
}

static int bio_core_puts(BIO *bio, const char *str)
{
    return xor_prov_bio_puts(BIO_get_data(bio), str);
}
...
{
    BIO_set_init(bio, 0);
    xor_prov_bio_free(BIO_get_data(bio));

    return 1;
}

static BIO_METHOD *xor_bio_prov_init_bio_method(void)
{
...
static BIO *xor_bio_new_from_core_bio(PROV_XOR_CTX *provctx,
                                      OSSL_CORE_BIO *corebio)
{
    ...
    if ((outbio = BIO_new(corebiometh)) == NULL)
        return NULL;
    if (!xor_prov_bio_up_ref(corebio)) {
        BIO_free(outbio);
        return NULL;
    }
...
static PROV_XOR_CTX *xor_newprovctx(OSSL_LIB_CTX *libctx)
{
    ...
    BIO_METHOD *corebiometh = xor_bio_prov_init_bio_method();

    if (prov_ctx == NULL)
        return NULL;

    if (corebiometh == NULL || libctx == NULL) {
        OPENSSL_free(prov_ctx);
	return NULL;
    }
    prov_ctx->libctx = libctx;
    prov_ctx->corebiometh = corebiometh;
    return prov_ctx;
}
...
static int xor_match(const void *keydata1, const void *keydata2, int selection)
{
    ...
    if (key1->tls_name && key2->tls_name)
        ok = ok & (!strcmp(key1->tls_name, key2->tls_name));
                          ...
                          OSSL_LIB_CTX *libctx, const char *propq)
{
	...
	key->hasprivkey = 1;
	/* XXX do not re-generate public part here to test
	 * https://github.com/openssl/openssl/pull/19505
        for (i = 0; i < XOR_KEY_SIZE; i++)
            key->pubkey[i] = key->privkey[i] ^ private_constant[i];
	key->haspubkey = 1;
	 */
...
XORKEY *xor_key_from_x509pubkey(const X509_PUBKEY *xpk,
                                 OSSL_LIB_CTX *libctx, const char *propq)
{
...
XORKEY *xor_key_from_pkcs8(const PKCS8_PRIV_KEY_INFO *p8inf,
                           OSSL_LIB_CTX *libctx, const char *propq)
{
    XORKEY *xork = NULL;
    const unsigned char *p;
...
static const OSSL_ALGORITHM tls_prov_keymgmt[] = {
    ...
    { XORSIGALG_HASH_NAME ":" XORSIGALG_HASH_LNAME,
                                  ...
                                  i2d_of_void k2d)
{
    ...
    if ((xpk = X509_PUBKEY_new()) == NULL
        ...
        || !X509_PUBKEY_set0_param(xpk, OBJ_nid2obj(key_nid),
                        V_ASN1_UNDEF, NULL, // as per logic in xor_meth.c in xor-openssl
                        der, derlen)) {
        ERR_raise(ERR_LIB_USER, ERR_R_MALLOC_FAILURE);
...
static int xorx_pki_priv_to_der(const void *vecxkey, unsigned char **pder)
{
    ...
    if (xorxkey == NULL || xorxkey->privkey == NULL
                    || xorxkey->pubkey == NULL
        ) {
        ERR_raise(ERR_LIB_USER, ERR_R_PASSED_NULL_PARAMETER);
        return 0;
...
// XOR provider uses NIDs generated at load time as EVP_type identifiers
                          ...
                          i2d_of_void *key2der)
{
    ...
    if (key == NULL || type <= 0) {
        ...
        BIO *out = xor_bio_new_from_core_bio(ctx->provctx, cout);
 ...
 *                                      except public key
 */
#define DO_ENC_type_specific_params_selection_mask DO_ENC_PARAMETERS_selection_mask
#define DO_ENC_type_specific_params(impl, type, output)                         \
    DO_ENC_PARAMETERS(impl, type, type_specific, output)
#define DO_ENC_type_specific_keypair_selection_mask                             \
    ( DO_ENC_PRIVATE_KEY_selection_mask | DO_ENC_PUBLIC_KEY_selection_mask )
#define DO_ENC_type_specific_keypair(impl, type, output)                        \
    DO_ENC_PRIVATE_KEY(impl, type, type_specific, output)                       \
    DO_ENC_PUBLIC_KEY(impl, type, type_specific, output)
#define DO_ENC_type_specific_selection_mask                                     \
    ( DO_ENC_type_specific_keypair_selection_mask                               \
      | DO_ENC_type_specific_params_selection_mask )
#define DO_ENC_type_specific(impl, type, output)                                \
    DO_ENC_type_specific_keypair(impl, type, output)                            \
    DO_ENC_type_specific_params(impl, type, output)
#define DO_ENC_type_specific_no_pub_selection_mask \
    ( DO_ENC_PRIVATE_KEY_selection_mask |  DO_ENC_PARAMETERS_selection_mask)
#define DO_ENC_type_specific_no_pub(impl, type, output)                         \
    DO_ENC_PRIVATE_KEY(impl, type, type_specific, output)                       \
    DO_ENC_type_specific_params(impl, type, output)
...
{
    BUF_MEM *mem = NULL;
    BIO *in = xor_bio_new_from_core_bio(provctx, cin);
                                   ...
                                   int operation)
{
    ...
    if (!xor_key_up_ref(vxorsig)) return 0;
    pxor_sigctx->sig = vxorsig;
    pxor_sigctx->operation = operation;
    if ( (operation==EVP_PKEY_OP_SIGN && !pxor_sigctx->sig->privkey) ||
         (operation==EVP_PKEY_OP_VERIFY && !pxor_sigctx->sig->pubkey)) {
        ERR_raise(ERR_LIB_USER, XORPROV_R_INVALID_KEY);
        return 0;
    }
    return 1;
                    ...
                    size_t sigsize, const unsigned char *tbs, size_t tbslen)
{
    ...
    unsigned int xor_sig_len = 0;
    int rv = 0;

    if (!xorkey || !xorkey->hasprivkey) {
     ...
     */
    if (!HMAC(EVP_sha1(), xorkey->privkey, XOR_KEY_SIZE,
              tbs, tbslen, &sig[0], &xor_sig_len)) {
                          ...
                          const unsigned char *tbs, size_t tbslen)
{
    ...
    unsigned int resiglen;
    int i;

    if (!xorkey || !xorkey->pubkey || sig == NULL || tbs == NULL) {
    ...
    if (!HMAC(EVP_sha1(), xorkey->privkey, XOR_KEY_SIZE,
              tbs, tbslen, &resignature[0], &resiglen)) {
        ERR_raise(ERR_LIB_USER, XORPROV_R_VERIFY_ERROR);
	return 0;
    }

    /* Now compare with signature passed */
    if (siglen != resiglen || memcmp(resignature, sig, siglen)) {
        ERR_raise(ERR_LIB_USER, XORPROV_R_VERIFY_ERROR);
	return 0;
    }
    return 1;
                                                     ...
                                                     ossl_unused void *provctx)
{
     ...
     * NOTE: Ideally we would check pxor_sigctx->flag_allow_md, but this is
     * problematic because there is no nice way of passing the
     * PROV_XORSIG_CTX down to this function...
...
static void tls_prov_teardown(void *provctx)
{
    ...
    OSSL_LIB_CTX_free(((PROV_XOR_CTX*)provctx)->libctx);
                      ...
                      void **provctx)
{
    ...
    *provctx = prov_ctx;
    ...
    /* Register BIO for use */
    if (!xor_prov_bio_from_dispatch(in))
        return 0;

    /* Fake signatures use HMAC and thus need default provider in NULL context */
     ...
     * key & cert generation...
     */
    if (!c_obj_create(handle, XORSIGALG_OID, XORSIGALG_NAME, XORSIGALG_LNAME)) {
