 ...
 * of the artificial "hmacsig" algorithm implemented in tls-provider
 */
static int create_cert_key(int idx, char *certfilename, char *privkeyfilename)
{
    ...
    if (!TEST_ptr(evpctx) ||
        ...
        !TEST_true(X509_gmtime_adj(X509_get_notBefore(x509), 0)) ||
        !TEST_true(X509_gmtime_adj(X509_get_notAfter(x509), 31536000L)) ||
        ...
        !TEST_ptr(f = fopen(privkeyfilename, "wb")) ||
        !TEST_true(PEM_write_PrivateKey(f, pkey, NULL, NULL, 0, NULL, NULL)) ||
        fclose(f) ||
        !TEST_ptr(f = fopen(certfilename, "wb")) ||
        !TEST_true(PEM_write_X509(f, x509)) ||
        fclose(f))
        ret = 0;
